/**
 * FOOTBALL DATA HUB PRO - v5.29.0 "THE GUARDIAN'S FOUNDATION"
 * 5 Moduli: ADMIN, NOMI, MATCH, CAMPIONATI, QUARANTENA.
 * Feature: Anomaly Quarantine, Webhook Trigger, Auto-Cleanup, Persistent Logs.
 * Fix: Ireland/Summer Leagues, 10.5px Font, ✖️ Buttons, No-Backtick UI.
 */
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (p === "/api/admin/fix-ids") return await handleFixIds(env, h);
      if (p === "/api/admin/reset") return await handleReset(request, env, h);
      
      // API QUARANTENA E LOGS
...
async function updateSignal(env, force = false) {
  if (force) {
    ...
    // WEBHOOK TRIGGER: Sveglia l'Engine (se configurato in wrangler.toml)
    if (env.ENGINE_URL) {
...
async function handleAddLeague(request, env, h) {
  ...
  await addLog(env, "Lega aggiunta/modificata: " + l.id);
...
async function handleDeleteLeague(request, env, h) {
  ...
  await addLog(env, "Lega archiviata: " + id, "warning");
  return new Response(JSON.stringify({ success: true }), { headers: h });
}
...
// --- ENGINE DOWNLOAD (QUARANTINE & AUTO-CLEANUP) ---
  ...
  try {
    ...
    try {
      if (matchIdsInFile.length > 0) {
        for (let i = 0; i < matchIdsInFile.length; i += 900) {
          const chunk = matchIdsInFile.slice(i, i + 900);
          const placeholders = chunk.map(() => "?").join(",");
          const checkArchivio = await env.DB.prepare("SELECT COUNT(*) as c FROM archivio_elaborato WHERE match_id IN (" + placeholders + ")").bind(...chunk).first();
          if (checkArchivio && checkArchivio.c > 0) { needsEngineReset = true; break; }
        }
      }
    } catch(e) {}
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (hId && aId) {
        ...
        // ANOMALY DETECTION: Quarantena se nazione non coincide
        const hTeam = teamsMap.get(hId), aTeam = teamsMap.get(aId);
        const hAnom = (hTeam.country !== league.country && !exceptions.has(hId + "_" + league.id));
        const aAnom = (aTeam.country !== league.country && !exceptions.has(aId + "_" + league.id));
    ...
    // AUTO-CLEANUP: Rimuove match orfani
...
async function handleTransfer(env, h) {
...
async function handleFixIds(env, h) { await env.DB.prepare("UPDATE matches SET id = season || '_' || div || '_' || home_team_id || '_' || away_team_id || '_' || REPLACE(date, '-', '') WHERE home_team_id IS NOT NULL AND away_team_id IS NOT NULL").run(); return new Response(JSON.stringify({ success: true }), { headers: h }); }
...
"    <title>GOLDBET DATABASE v5.29.0</title>",
...
"                var m = data[i]; var leg = getLega(m.div);",
